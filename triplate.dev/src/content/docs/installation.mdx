...
<Tabs syncKey="lang">
  <TabItem label="TypeScript">
    Published to npm as [`triplate`](https://www.npmjs.com/package/triplate).
    Ships as ES modules with bundled TypeScript types (plain JavaScript works
    too); requires Node 18+ (or any modern bundler / runtime with ESM).
  ...
  <TabItem label="Python">
    Published to PyPI as [`triplate`](https://pypi.org/project/triplate/);
    requires Python 3.10+. The core is dependency-free. Install the optional
    `rdflib` extra only if you want to pass `rdflib` terms to the `term` type:
  ...
  <TabItem label="Java">
    Published to Maven Central under `dev.triplate`; requires Java 17+ (the
    library uses records and sealed types). No runtime dependencies.
...
- Browse the examples for [Query](/examples/query/), [Data](/examples/data/)
  and [Publishing](/examples/publishing/) — real-world, use-case-based recipes
  covering every syntax element.
